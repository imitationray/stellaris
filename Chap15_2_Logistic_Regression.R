...
pred 
range(pred, na.rm = T) # 0.001653842 0.995227133
summary(pred, na.rm = T)
str(pred)
...
cat('accuracy =', acc) # accuracy = 0.8454545

no <- tab[1,1]/(tab[1,1] + tab[1,2])
no # 0.9058824
yes <- tab[2,1]/(tab[2,1] + tab[2,2])
yes # 0.3333333
...
# 3) 특이도 : NO -> NO
tab[1,1] / (tab[1,1] + tab[1,2]) # 0.9058824
...
range(model$fitted.values) # 3.610502e-228  1.000000e+00

names(model)
model$fitted.values
range(model$fitted.values) # 3.610502e-228  1.000000e+00
str(model$fitted.values) # num [1:105, 1:3] -> matrix
model$fitted.values[1,] # versicolor : 예측치
train[1,] # versicolor : 관측치

# cut off
y_pred <- ifelse(y_pred[,1] >= 'setosa', )
