...
rect.hclust(hc,
            k = 3,
            border = "red") # 3개 그룹 선정, 선 색 지정

# 각 그룹별 서브셋 만들기
g1 <- subset(body,
             번호==15| 번호==1| 번호==4| 번호==8 | 번호==10)
g1
g2 <- subset(body,
             번호==11| 번호==3| 번호==5| 번호==6 | 번호==14)
g2
g3 <- subset(body,
             번호==2| 번호==9| 번호==7| 번호==12 | 번호==13)
g3
...
# 2) 계층적 군집분석(탐색적 분석)
result <- hclust(dist(mydia),
                 method = "average") # 평균거리 이용 
result
...
corrgram(mydia[, -5],
         upper.panel = panel.conf) # 수치(상관계수) 추가(위쪽)
...
library(NbClust)
data("iris")
iris_max <- as.matrix(iris[-5]) # 105 x 4
