...
vname <- unlist(name)
xname <- str_extract_all(EMP, '[가-힣]{3}')
xname <- unlist(xname)
vname <- str_replace_all(vname, '[가-힣]', '')
frame <- data.frame(xname, vname)
# 함수 정의 : for()함수 이용 예  
emp_pay <- function(frame){
  xmean <- mean(as.numeric(vname))
  cat('전체 급여 평균 :', xmean, '\n')
  cat('평균 이상 급여 수령자\n')
...
Bug_Metrics_Software[1,,2]
Bug_Metrics_Software[,,2]
apply(Bug_Metrics_Software[,,2], 2, summary)
