...
tran <- read.transactions("tran.txt",
                          format = "basket",
                          sep = ",")
tran
...
rule <- apriori(tran,
                parameter = list(supp = 0.3, conf = 0.1)) # 16 rule
rule <- apriori(tran,
                parameter = list(supp = 0.1, conf = 0.1)) # 35 rule 
inspect(rule) # 규칙 보기

# 지지도, 신뢰도, maxlen 인수  
help("apriori") # support 0.1, confidence 0.8, and maxlen 10 
rule <- apriori(tran) 
rule <- apriori(tran, parameter = list(supp = 0.1, conf = 0.1, maxlen = 10)) 
inspect(rule) 
...
setwd("C:/ITWILL/2_Rwork/Part-IV")
stran <- read.transactions("demo_single",
                           format = "single",
                           cols = c(1, 2)) 
inspect(stran)

# <실습> 중복 트랜잭션 객체 생성
stran2 <- read.transactions("single_format.csv",
                            format = "single",
                            sep = ",",
                            cols = c(1, 2),
                            rm.duplicates = T)
...
btran <- read.transactions("demo_basket",
                           format = "basket",
                           sep = ",") 
