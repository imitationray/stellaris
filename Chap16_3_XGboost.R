...
# 4. xgboost model 생성 
xgb_iris_model <- xgboost(data = dtrain,
                          max_depth = 2,
                          eta = 1,
                          nthread = 2,
                          nrounds = 2,
...
# 1. y변수 -> binary(Species : num)
iris_df$Species <- ifelse(iris_df$Species == 'setosa',
                          0,
                          ifelse(iris_df$Species == 'versicolor',
                                 1,
                                 2)
                          )
str(iris_df)
...
train_x <- as.matrix(train[-5]) # metrix
train_y <- train$Species # vector

dmtrix <- xgb.DMatrix(data = train_x,
                      label = train_y)
...
cat("accuracy =", acc) # accuracy = 0.9666667

mean_err <- mean(pred != test_y)
cat('mean error = ', mean_err) # mean error =  0.03333333
...
# y : 1번 칼럼
# x : 2~4번 칼럼
train_x <- as.matrix(train[, -c(1, 5)]) # metrix
train_y <- train$Sepal.Length # vector

dmtrix <- xgb.DMatrix(data = train_x,
                      label = train_y)

xgb_iris_model3 <- xgboost(data = dmtrix,
                           max_depth = 2,
                           eta = 1,
                           nthread = 2,
                           nrounds = 3,
...
pred
range(pred) # 2.786015 3.457107 : 숫자 예측

# 5. model 평가 : MSE
mse <- mean((test_y - pred)**2)
cat('MSE =', mse) # MSE = 2.476664
