...
iris_model <- lm(formula = Sepal.Length ~ Sepal.Width + Petal.Length,
                 data = train)
iris_model
summary(iris_model)
...
# 회기모델 생성
insurance2 <- insurance[, -c(5:6)] # 흡연 여부, 지역 제외
head(insurance2)
...
# [해석] 여성이 남성에 비하여 의료비를 덜 지출하는 측면을 보인다. (-1321.7)
...
# [해석] 흡연자가 비흡연자에 비하여 23616 수준의 의료비 증가
