...
#   -> root node : 전체크기, setosa(68) 제외 -> 가장많은 비율 label(37)
# 2) Petal.Length< 2.45 37  0 setosa (1.00000000 0.00000000 0.00000000) *
#   -> left node : 분류조건 -> (37개 0개) 가장 많은 비율 label (각 label 분류 비율) 단노드 유무 
# 3) Petal.Length>=2.45 68 33 virginica (0.00000000 0.48529412 0.51470588)  
#   -> right node : 분류조건 -> 68개(전체개수), 33(오분류)
...
# 3 0.0100000      2 0.02941176 0.02941176 0.02059824 -> 가장 오류가 적다.
# 과적합 발생 -> 0.45 조정
...
cross <- cvFolds(n = nrow(iris),
                 K = 3, R = 1,
                 type = 'random') # K = 3 : d1 = 50, d2 = 50, d3 = 50
